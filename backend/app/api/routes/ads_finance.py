...
def demo_auto_topup_total(
    ...
    value = (
        db.query(
            func.coalesce(
                func.sum(AdFinanceTransaction.amount),
                0,
            )
        )
        .filter(
            AdFinanceTransaction.ad_account_id == account_id,
            AdFinanceTransaction.transaction_type == "DEMO_AUTO_TOPUP",
            AdFinanceTransaction.status == "SUCCESS",
            AdFinanceTransaction.created_at >= start,
            AdFinanceTransaction.created_at < end,
        )
        .scalar()
    )
...
@router.post("/accounts/{account_id}/demo-auto-check")
def demo_auto_rule_check(
    account_id: str,
    db: Session = Depends(get_db),
    user: User = Depends(
